...
def generar_tabla(estimado_1rm, formula, ejercicio=None):
    ...
    for reps in range(1, 11):
        if reps == 1:
            ...
            if ejercicio_factor:
                factor = rep_factors[ejercicio_factor].get(
                    reps, 1 / (1 + 0.0333 * reps)
                )
    ...
    if ejercicio in ["Dominadas con lastre", "Fondos con lastre"]:
        with col1:
            peso_corporal = st.number_input(
                "Tu peso corporal (kg)", min_value=40, step=1, value=70
            )
        with col2:
        ...
        with col1:
            peso_total = st.number_input(
                "Peso levantado (kg)", min_value=20, step=1, value=100
            )
...
if peso_total > 0 and reps > 0:
    ...
    df_consolidada = df_merge[["Reps", "Promedio"]].rename(
        columns={"Promedio": "Peso Estimado (kg)"}
    )
